...
const SelectDuplo = () => {
    ...
    const handleOptionEscolaChange = (option) => {
        ...
        const municipio = selectedOption.value
        const escola = option.value
        router.push(`/Escola/${municipio}/${escola}`)  // /${selectedOption}/${option}

        // window.location.href = `/Escola/${selectedOption}/${option}`;
        // document.getElementById('escola').innerHTML = `Escola selecionada: ${option.label}`;
    };
                ...
                (
                    <>
                        ...
                        <Select
                            ...
                            menuPosition="fixed" // Adjust the menu position (fixed, absolute, relative)
                            styles={customStyles}
                            placeholder={`clique para selecionar uma instituição`}  //  selecione uma instituição em ${selectedOption.value}
