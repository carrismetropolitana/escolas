...
function toTitleCase(str) {
  const upperCaseExceptions = ['EB', 'ITS', '(ITS)', 'EN', 'EDP'];
    ...
    if (upperCaseExceptions.includes(match.toUpperCase())) {
      ...
      return match.replace(/\w\S*/g, function (innerMatch) {
        if (/^ç/i.test(innerMatch)) {
          ...
          return lowerCaseExceptions.includes(innerMatch.toUpperCase()) ? innerMatch.toLowerCase() : innerMatch.charAt(0).toUpperCase() + innerMatch.substr(1).toLowerCase();
        }
      });
    }
  });
    ...
    <div className={`${!isMap ? styles.stop : ''} ${stop.routes.length < 5 ? styles.stopPoucasLinhas : ''}`}>
      ...
      {stop && stop.routes && Array.isArray(stop.routes)
        ? stop.routes.map((route, route_index) => (
            ...
            `}
            >
              <a href={`https://www.carrismetropolitana.pt/horarios/?route_short_name=${route.route_short_name}&date=&route_id=${route.route_id}&stop_id=${stop.stop_id}`} target="_blank">
                <span className={styles.numero} style={{ backgroundColor: route.route_color }}>
                  {route.route_short_name}
                </span>
              </a>

              <span className={styles.routeName}>{toTitleCase(route.route_long_name)}</span>
            </div>
          ))
        : null}
    </div>
